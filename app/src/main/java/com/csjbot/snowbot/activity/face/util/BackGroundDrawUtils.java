 ...
 */
public class BackGroundDrawUtils {
    private static volatile boolean onSecondAfter = false;
    ...
    public interface GuidePositiveCallback {
        void callback(int gender);
    }

    public void setCallback(GuidePositiveCallback callback) {
        BackGroundDrawUtils.callback = callback;
    }

    private static Map<Integer, User> userMap = new HashMap<>();
    private static List<User> userList = new ArrayList<>();
    private static FindRegisteredUserInterface findRegisteredUserInterface;
    private static GuidePositiveCallback callback;
...
//    private static final int OFFSET_Y = 75;
    ...
    public static void drawAnim(List<YMFace> faces, View outputView, float scale_bit,
        ...
        if (Constants.isGuideReco && Constants.isGuidePositive) {
            // 主动导览 && 是导览的识别
            if (faces != null && faces.size() > 0) {
                if (faces.get(0).getAge() > 0) {
                    int gender = faces.get(0).getGender();
//                    Csjlogger.debug("gender: " + gender);
                    if (!onSecondAfter) {
                        if(!isSend) {
                        ...
                        onSecondAfter = false;
                        isSend = false;
                        if (personRecoCallback != null) {
                            personRecoCallback.callback(faces.get(0).getGender());
                        }
                    }
                }
//                Csjlogger.debug("gender: " + gender);
...
//                    sb.append("trackId : " + ymFace.getTrackId() + " ");
                    ...
                    if (personId > 0 && userMap.containsKey(personId)) {
                        ...
                        if (findRegisteredUserInterface != null && !Constants.isGuideReco) {
...
//                    canvas.drawText(fps, 20, viewH * 3 / 17, paint);
    ...
    public static PersonRecoCallback personRecoCallback;

    public interface PersonRecoCallback {
        void callback(int gender);
    }

    public static void setPersonRecoCallback(PersonRecoCallback p) {
        BackGroundDrawUtils.personRecoCallback = p;
    }
