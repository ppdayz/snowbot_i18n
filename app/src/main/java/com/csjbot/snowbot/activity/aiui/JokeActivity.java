...
import com.csjbot.snowbot.bean.aiui.JokeBean;
...
public class JokeActivity extends AIUIActivity implements ExoPlayerBean.Listener {
    ...
    private String data, answerText;
    private List<JokeBean.ResultBean> jokeList = null;
    ...
    protected void onNewIntent(Intent intent) {
        ...
        jokeDoData(intent.getStringExtra("data"));
    }

    private void jokeDoData(String data) {
        pauseCmd = false;
        speakFinished = false;
        releasePlayer();

        JokeBean jokeBean = JSON.parseObject(data, JokeBean.class);
        jokeList = jokeBean.getResult();
        uri = Uri.parse(jokeList.get(0).getMp3Url());
    ...
    public boolean onAIUIEvent(AIUIEvent event) {
        ...
        switch (event.getTag()) {
            case EventsConstants.AIUIEvents.MUSICX_MUSIC_CRTL:
                jokeDoControl((String) event.data);
                break;
            default:
    ...
    private void jokeDoControl(String action) {
        if (action != null) {
            switch (action) {
