...
public class CamParaUtil {
    ...
    public Size getPropPreviewSize(List<Size> list, float th, int minWidth) {
        ...
        for (Size s : list) {
            if ((s.width >= minWidth) && equalRate(s, th)) {
                Csjlogger.info(TAG, "PreviewSize:w = " + s.width + "h = " + s.height);
                break;
            }
            i++;
        }
    ...
    public Size getPropPictureSize(List<Size> list, float th, int minWidth) {
        ...
        for (Size s : list) {
            if ((s.width >= minWidth) && equalRate(s, th)) {
                Csjlogger.info(TAG, "PictureSize : w = " + s.width + "h = " + s.height);
                break;
            }
            i++;
        }
    ...
    public void printSupportPreviewSize(Camera.Parameters params) {
        ...
        for (int i = 0; i < previewSizes.size(); i++) {
            ...
            Csjlogger.info(TAG, "previewSizes:width = " + size.width + " height = " + size.height);
        }
    ...
    public void printSupportPictureSize(Camera.Parameters params) {
        ...
        for (int i = 0; i < pictureSizes.size(); i++) {
            ...
            Csjlogger.info(TAG, "pictureSizes:width = " + size.width
                    + " height = " + size.height);
        }
    }
    ...
    public void printSupportFocusMode(Camera.Parameters params) {
        ...
        for (String mode : focusModes) {
            Csjlogger.info(TAG, "focusModes--" + mode);
        }
    }
}
