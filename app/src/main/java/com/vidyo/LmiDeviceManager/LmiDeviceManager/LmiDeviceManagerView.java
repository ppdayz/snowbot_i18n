...
//					setZOrderOnTop(true);
	...
	private void dumpEvent(MotionEvent event) {
	   ...
	   sb.append("]" );
	   Csjlogger.debug(TAG, sb.toString());
	}
	...
	private static class ContextFactory implements
		...
		public EGLContext createContext(EGL10 egl, EGLDisplay display,
				EGLConfig eglConfig) {
			Csjlogger.info(TAG, "creating OpenGL ES 2.0 context");
	...
	private static class ConfigChooser implements
		...
		public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display,
			...
			for (EGLConfig config : configs) {
				...
				if (r == mRedSize && g == mGreenSize && b == mBlueSize
						&& a == mAlphaSize) {
					Csjlogger.info(TAG, String.format("Using configId: %d", configId));
					return config;
				}
			}
			return null;
		...
		private void printConfigs(EGL10 egl, EGLDisplay display,
			...
			Csjlogger.info(TAG, String.format("%d configurations", numConfigs));
			for (int i = 0; i < numConfigs; i++) {
				Csjlogger.info(TAG, String.format("Configuration %d:\n", i));
		...
		private void printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
			...
			for (int i = 0; i < attributes.length; i++) {
				...
				if (egl.eglGetConfigAttrib(display, config, attribute, value)) {
					Csjlogger.info(TAG, String.format("  %s: %d\n", name, value[0]));
				} else {
	...
	private static class Renderer implements GLSurfaceView.Renderer {
		...
		public void onSurfaceChanged(GL10 gl, int width, int height) {
			Csjlogger.info(TAG, "onSurfaceChanged()");
			if (callback_ != null)
				callback_.LmiDeviceManagerViewResize(width, height);
		}

		public void onSurfaceCreated(GL10 gl, EGLConfig config) {
			Csjlogger.info(TAG, "onSurfaceCreated()");
