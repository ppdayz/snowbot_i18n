...
public class LmiAudioCapturer {
	...
	private static void setEchoCancel(AudioRecord record) {
		if (Build.VERSION.SDK_INT >= 16) {
			...
			Csjlogger.info(TAG, "AcousticEchoCanceler.isAvailable() = " + aecIsAvailable);
			if (aecIsAvailable) { 
				...
				Csjlogger.info(TAG, "AEC_enabled=" + aec.getEnabled() + ", hasControl=" + aec.hasControl());
			}
			
			boolean agcIsAvailable = AutomaticGainControl.isAvailable();
			Csjlogger.info(TAG, "AutomaticGainControl.isAvailable() = " + agcIsAvailable);
			if (agcIsAvailable) { 
				AutomaticGainControl agc = AutomaticGainControl.create(record.getAudioSessionId());
				if(!agc.getEnabled()) {
					agc.setEnabled(true);
				}
				Csjlogger.info(TAG, "AGC_enabled=" + agc.getEnabled() + ", hasControl=" + agc.hasControl());
			}
			
			boolean nsIsAvailable = NoiseSuppressor.isAvailable();
			Csjlogger.info(TAG, "NoiseSuppressor.isAvailable() = " + nsIsAvailable);
			if (nsIsAvailable) {
				...
				Csjlogger.info(TAG, "NoiseSuppressor_enabled=" + ns.getEnabled() + ", hasControl=" + ns.hasControl());
			}
	...
	private static void logEchoCancelState(AudioRecord record) {
		if (Build.VERSION.SDK_INT >= 16) {
			...
			Csjlogger.info(TAG, "AcousticEchoCanceler.isAvailable() = " + isAvailable);
		}
	}
	...
	private static class DeviceThread extends Thread {
		...
		public void run() {
			...
			Csjlogger.info(TAG, "Microphone Starting");
			try {
...
//						AudioFormat.CHANNEL_CONFIGURATION_MONO,
				...
				Csjlogger.info(TAG, "MinFrameSize: " + String.format("%d", minframeSize)
				...
				Csjlogger.info(TAG, "Starting audio capture. Rate: "
			...
			Csjlogger.info(TAG, "Microphone Stopping");
			record.stop();
			record.release();
			Csjlogger.info(TAG, "Microphone Stopped");
			micIsRunning = false;
	...
	public byte[] aquireFrame() {
		...
		if (readyFrames != null) {
			try {
				...
				Csjlogger.info(TAG, "Frames are not ready " + e.toString());
			}
	//		if (frame == null)
