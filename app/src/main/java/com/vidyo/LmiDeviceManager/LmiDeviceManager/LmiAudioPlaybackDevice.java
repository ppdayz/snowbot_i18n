...
public class LmiAudioPlaybackDevice {
	...
	private static class DeviceThread extends Thread {
		...
		public void run() {
			...
			Csjlogger.info(TAG, "Speaker Starting");
			try {
			...
			try {
				...
				Csjlogger.info(TAG, "Starting audio playback. Rate: "
			...
			for (i = 0; i < numberOfFrames; i++) {
				...
				try {
					...
					Csjlogger.error(TAG, "unable to add frame" + e.toString());
				}
			}
			...
			while (running == true) {
				...
				try {
					...
					Csjlogger.error(TAG, "No frames avaialble " + e.toString());
					continue;
				}
			...
			Csjlogger.info(TAG, "Speaker Stopping");
			track.stop();
			track.release();
			...
			Csjlogger.info(TAG, "Speaker Stopped");
		}
	...
	public byte[] aquireFrame() {
		...
		if (readyFrames != null) {
			try {
				...
				Csjlogger.error(TAG, "Frames are not ready " + e.toString());
	...
	public void releaseFrame(byte[] frame) {
		if (frames != null) {
			try {
				...
				Csjlogger.error(TAG, "unable to release captured frame" + e.toString());
			}
		}
	}
...
//		{ "model", "device", },
	...
	private static boolean setupSpeakerNeedsVolumeIncrease(AudioTrack track) {
		...
		Csjlogger.debug(TAG, "Manufacturer=" + manufacturer + ", Device=" + device + ", Model=" + model);
		...
		synchronized (activeTrack) {
			...
			for (int i=0; i<volumeBoostDevices.length; i++) {
				if (manufacturer.equalsIgnoreCase(volumeBoostDevices[i][0]) &&
					...
					Csjlogger.debug(TAG, "Device needs a volume boost!");
					needsIncrease = true;
					break;
				}
			}
			...
			if (needsIncrease) {
				...
				if (currentEqualizer != null) {
					...
					for (short level : savedEqualizerSetting.bandLevels) {
						Csjlogger.debug(TAG, "Equalizer: bandLevels[]=" + level);
					}
				} else {
				...
				if (!currentlyUsingSpeaker) {
					...
					try {
						...
						if (levelRange != null && levelRange.length == 2) {
							Csjlogger.debug(TAG, "Equalizer: BandLevelRange =" + levelRange[0] + " to " + levelRange[1]);
						...
						Csjlogger.error(TAG, "getBandLevelRange() exception, ignoring");
					}
				}
			} else {
				if (currentlyUsingSpeaker) {
