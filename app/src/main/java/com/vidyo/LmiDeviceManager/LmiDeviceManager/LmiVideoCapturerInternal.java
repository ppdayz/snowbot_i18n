 ...
 */
package com.vidyo.LmiDeviceManager.LmiDeviceManager;
...
public class LmiVideoCapturerInternal extends SurfaceView implements Camera.PreviewCallback, SurfaceHolder.Callback {
	...
	public LmiVideoCapturerInternal(final Context context, final Activity activity, final String id) {
		super(context);
		Csjlogger.info(TAG, "constructor for " + id);
		active = true;
		...
		Csjlogger.info(TAG, "constructor exit");
	}

	public LmiVideoCapturerCapability[] getCapabilities() {
		Csjlogger.info(TAG, "getCapabilities()");
...
/*	public LmiVideoCapturerCapability[] getCapabilities(boolean anyFormat) {
		Csjlogger.info(TAG,"getCapabilities(anyFormat:"+anyFormat+")");
...
*/
    ...
    private void setAdvancedCameraParameters() {
        ...
        Csjlogger.info(TAG, "Setting advanced camera parameters");
		...
		if (Build.MANUFACTURER.toLowerCase().equalsIgnoreCase("amazon") && Build.DEVICE.equalsIgnoreCase("d01e")){
					Csjlogger.info(TAG, "support for autofocus is off for amazon kindle HD");
		}else{
	...
	if (Build.MANUFACTURER.toLowerCase().equalsIgnoreCase("samsung") &&
			...
			Build.MODEL.equalsIgnoreCase("Nexus 10")){
					Csjlogger.info(TAG, "video-stabilization-supported is off for Nexus 10");
		}else{ 
	...
	public boolean start(String format, int width, int height, int frameRate) {
		...
		stopping = false;
		Csjlogger.info(TAG, "	public boolean start(String format:" + format + ", int width:" + width + ", int height:" + height + ", int frameRate:" + frameRate + ")");
		...
		try {
			...
			Csjlogger.debug(TAG, "Starting Camera. format: " + format + " width:" + width + " height:" + height + " min-frameRate: " + minFrameRate + " max-frameRate: " + maxFrameRate);
			...
			Csjlogger.debug(TAG, "Pixel format: " + fourCC);
			...
			Csjlogger.info(TAG, "Camera Started");
		} catch (Exception ex) {
			if (camera != null)
				camera.release();
	...
	private void destroyCamera(){
		Csjlogger.info(TAG, "destroyCamera");
		synchronized(this) { 
			if(camera != null){
				camera.stopPreview();
				...
				camera.release();
				camera = null;	
				Csjlogger.info(TAG, "camera destroyed");
			}
		}
	}
	...
	public void stop() {
		...
		if (camera != null) {
			...
			destroyCamera();	
			Csjlogger.info(TAG, "stop: Camera stopped");
		}
		
		Csjlogger.info(TAG, "stop: draining frames");
		drainFrames();
	...
	public void surfaceCreated(SurfaceHolder holder) {
			Csjlogger.info(TAG, "	surfaceCreated");
		...
		Csjlogger.info(TAG, "Camera Preview Starting");
		...
		Csjlogger.info(TAG, "Camera Preview Started");
	}

	public void surfaceDestroyed(SurfaceHolder holder) {
				Csjlogger.info(TAG, "	surfaceDestroyed");
		...
		Csjlogger.info(TAG, "Camera Stopping");
		camera.stopPreview();
		Csjlogger.info(TAG, "Camera Preview Stopped");
	...
	private void enumerateCapabilities(boolean anyFormat) {
		...
		Csjlogger.info(TAG, "enumerateCapabilities entry");
		...
		Camera camera = null;
		Camera.Parameters parameters;
		Csjlogger.info(TAG, "Enumerating camera capabilities");
		try {
			if (deviceId == LmiVideoCapturerManager.BACK) {
				camera = Camera.open();
			} else if (deviceId == LmiVideoCapturerManager.FRONT) {
			...
			for (Integer previewFormat : formatList) {
				for (Size previewSize : sizeList) {
					for (int[] sfps : supportedPreviewMaxFps) {
						Csjlogger.debug(TAG, "Found Configuration format for  " + Integer.toString(deviceId) + ": " + Integer.toString(previewFormat) + " size: " + Integer.toString(previewSize.width) + "x" + Integer.toString(previewSize.height) + " min-sample-rate: " + Integer.toString(sfps[0] / 1000) + " max-sampling-rate: " + Integer.toString(sfps[1] / 1000));
						numCapabilities++;
					}
				}
// 				if(anyFormat){
...
// 				}
		...
		Csjlogger.debug(TAG, "enumerateCapabilities b4 release");

			camera.release();
		Csjlogger.debug(TAG, "enumerateCapabilities after release");
		...
		Csjlogger.debug(TAG, "enumerateCapabilities b4 save");

		saveEnumratedCapabilities();
		Csjlogger.debug(TAG, "enumerateCapabilities exit");
	...
	private Camera getFrontCamera() {
		Camera camera;
		Camera.Parameters parameters;
		Csjlogger.info(TAG, "	getFrontCamera");
	...
	private Camera getCameraUsingAPI9(int id) {
		Csjlogger.info(TAG, "getCameraUsingAPI9()");
		try {
			Method open = Class.forName("android.hardware.Camera").getDeclaredMethod("open", new Class[]{int.class});
	...
	private void getOrientationUsingAPI9(int id) {
		Csjlogger.info(TAG, "getOrientationUsingAPI9()");
	...
	public static void forceOrientationStart(int camera, int orientation) {
		if (orientation != ORIENTATION_UP && orientation != ORIENTATION_DOWN &&
			...
			Csjlogger.debug(TAG, "forceOrientationStart: invalid orientation (" + orientation + ")");
		}
		...
		if (camera == LmiVideoCapturerManager.FRONT){
			...
			Csjlogger.debug(TAG, "forceOrientationStart: invalid device (" + camera + ")");
		}
	}
	...
	public static void forceOrientationStop(int camera) {
		if (camera == LmiVideoCapturerManager.FRONT){
			...
			Csjlogger.debug(TAG, "forceOrientationStop: invalid device (" + camera + ")");
		}
	}
	...
	public static void forceMirrorStart(int camera, boolean mirrored) {
		if (camera == LmiVideoCapturerManager.FRONT){
			...
			Csjlogger.debug(TAG, "forceMirrorStart: invalid device (" + camera + ")");
		}
	}
	...
	public static void forceMirrorStop(int camera) {
		if (camera == LmiVideoCapturerManager.FRONT){
			...
			Csjlogger.debug(TAG, "forceMirrorStop: invalid device (" + camera + ")");
		}
	}
	...
	private void initializeBuffers() {
		...
		Csjlogger.debug(TAG, "Using callback buffers");
	...
	private void setPreviewCallbackWithBuffer(Object previewCallbackObj) {
		try {
			...
			if (spcwb != null) {
				...
				Csjlogger.info(TAG, "setPreviewCallbackWithBuffer: Called method");
			} else {
				Csjlogger.info(TAG, "setPreviewCallbackWithBuffer: Did not find method");
			}

		} catch (Exception e) {
			/* TODO Auto-generated catch block */
			Csjlogger.info(TAG, "setPreviewCallbackWithBuffer error" + e.toString());
		}
	}
	...
	public byte[] aquireFrame() {
		...
		if (!stopping) {
			try {
				...
				Csjlogger.info(TAG, "No frames avaialble " + e.toString());
			}
			if (stopping)
				return null;
		}
	...
	private void loadEnumratedCapabilities(){
		...
		if(infoLen>0){
			...
			for(i=0; i<infoLen; i++){
				...
				Csjlogger.debug(TAG, "Loading Configuration format for  " + Integer.toString(deviceId) + ": " + format + " size: " + Integer.toString(width) + "x" + Integer.toString(height) + " min-sample-rate: " + Integer.toString(rateMin) + " max-sampling-rate: " + Integer.toString(rateMax));
